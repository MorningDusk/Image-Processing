...
int main(int argc, char *argv[])
	...
	imin = 1000.; // stores minimum Fourier spectrum value
    	for(i=0; i< number_of_pixels; i++) {
			fmax = sqrt(pow(complex_data[i].re, 2) + pow(complex_data[i].im, 2));
			fmax = (fmax < 0) ? -fmax : fmax;
			imax = (imax < fmax) ? fmax : imax;
			imin = (imin > fmax) ? fmax : imin;
		}
	printf("fmax = %f  \n",fmax);
	...
	for(i=0; i< number_of_pixels; i++) {
		...
		ftmp = log(1.0 + ftmp);
		buffer[i] = (int)(255. * ftmp / fmax);
	}

	for(i=0; i<rows; i++) {
		for(j=0; j<cols; j++) {
		/*영상의 중앙 부분을 원점으로 만들도록  x,y의 좌표를 재계산한다.*/
          	// Coding is required !!!!
          	// Move center position!
          	//		
			x = (rows / 2 - i) % rows;
			y = (cols / 2 - j) % cols;
			obuffer[i * cols + j] = buffer[x * cols + y];
		}
	} 
